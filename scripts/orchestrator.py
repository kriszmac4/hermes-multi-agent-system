...
Spawns hermes profiles in parallel, collects results, synthesizes output.

Usage:
    python3 orchestrator.py "Keress szállásokat Zürichben 600€ körül"
    python3 orchestrator.py --profiles research,dev "Implementáld a foglalás API-t"
    python3 orchestrator.py --parallel research,dev "Kutatás + kódolás párhuzamosan"
"""
...
def select_profiles(task: str, explicit_profiles: Optional[list] = None) -> list:
    """Select which profiles to use based on task content."""
...
def clean_hermes_output(output: str) -> str:
    """Remove Hermes TUI artifacts from output."""
    lines = output.split('\n')
    cleaned = []
    skip_until_empty = False
    
    for line in lines:
        # Skip TUI box drawing
        ...
        if '⣀' in line or '⣴' in line or '⣿' in line or '⡀' in line:
            skip_until_empty = True
            continue
        # Skip tool list lines
        if 'Available Tools' in line or 'browser:' in line or 'terminal:' in line:
            skip_until_empty = True
            continue
        # Skip Hermes banner lines
        if 'Hermes Agent v' in line or 'Project:' in line or 'Python:' in line:
            skip_until_empty = True
            continue
        ...
        if skip_until_empty:
            continue
        # Skip warning lines
        if '⚠' in line or 'hermes setup' in line.lower():
            continue
        # Keep actual content
        cleaned.append(line)
...
async def run_profile(profile: str, task: str) -> TaskResult:
    """Run a single hermes profile with the given task."""
    start = time.time()
...
async def orchestrate(task: str, profiles: list, parallel: bool = True) -> dict:
    """Orchestrate task across multiple profiles."""
    ...
    if parallel:
        ...
        for profile in profiles:
            ...
            if not result.success:
                print(f"⚠️ {profile} failed, continuing...")
